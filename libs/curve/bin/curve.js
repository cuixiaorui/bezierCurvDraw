...
function fitCurve(points, maxError, progressCallback) {
    ...
    points.forEach(function (point) {
        if (!Array.isArray(point) || point.some(function (item) { return typeof item !== 'number'; })
    ...
    points = points.filter(function (point, i) {
        return i === 0 || !point.every(function (val, j) { return val === points[i - 1][j]; });
    });
    if (points.length < 2) {
        return [];
    }
...
function fitCubic(points, leftTangent, rightTangent, error, progressCallback) {
    ...
    _a = generateAndReport(points, u, u, leftTangent, rightTangent, progressCallback), bezCurve = _a[0], maxError = _a[1], splitPoint = _a[2];
    if (maxError < error) {
        return [bezCurve];
    }
    ...
    if (maxError < (error * error)) {
        ...
        for (i = 0; i < MaxIterations; i++) {
            ...
            _b = generateAndReport(points, u, uPrime, leftTangent, rightTangent, progressCallback), bezCurve = _b[0], maxError = _b[1], splitPoint = _b[2];
            if (maxError < error) {
                return [bezCurve];
            }
    ...
    if (centerVector.every(function (val) { return val === 0; })) {
        //[x,y] -> [-y,x]: http://stackoverflow.com/a/4780141/1869660
        centerVector = maths.subtract(points[splitPoint - 1], points[splitPoint]);
        _c = [-centerVector[1], centerVector[0]], centerVector[0] = _c[0], centerVector[1] = _c[1];
    }
...
function generateAndReport(points, paramsOrig, paramsPrime, leftTangent, rightTangent, progressCallback) {
    ...
    _a = computeMaxError(points, bezCurve, paramsOrig), maxError = _a[0], splitPoint = _a[1];
    if (progressCallback) {
        progressCallback({
            bez: bezCurve,
            points: points,
...
function reparameterize(bezier, points, parameters) {
    ...
    return results;
    //*/
    return parameters.map(function (p, i) { return newtonRaphsonRootFind(bezier, points[i], p); });
}
;
/**
...
function chordLengthParameterize(points) {
    ...
    u = u.map(function (x) { return x / prevU; });
    return u;
}
;
/**
...
var mapTtoRelativeDistances = function (bez, B_parts) {
    ...
    B_t_dist = B_t_dist.map(function (x) { return x / sumLen; });
    return B_t_dist;
};
...
var maths = (function () {
    ...
    maths.mulItems = function (items, multiplier) {
        return items.map(function (x) { return x * multiplier; });
    };
    maths.mulMatrix = function (m1, m2) {
        //https://en.wikipedia.org/wiki/Matrix_multiplication#Matrix_product_.28two_matrices.29
        //Simplified to only handle 1-dimensional matrices (i.e. arrays) of equal length:
        return m1.reduce(function (sum, x1, i) { return sum + (x1 * m2[i]); }, 0);
    };
    //Only used to subract to points (or at least arrays):
    //  subtract = logAndRun(math.subtract);
    maths.subtract = function (arr1, arr2) {
        return arr1.map(function (x1, i) { return x1 - arr2[i]; });
    };
    //add = logAndRun(math.add);
    maths.addArrays = function (arr1, arr2) {
        return arr1.map(function (x1, i) { return x1 + arr2[i]; });
    };
    maths.addItems = function (items, addition) {
        return items.map(function (x) { return x + addition; });
    };
    //var sum = logAndRun(math.sum);
    maths.sum = function (items) {
        return items.reduce(function (sum, x) { return sum + x; });
    };
    //chain = math.chain;
    //Only used on two arrays. The dot product is equal to the matrix product in this case:
    //  dot = logAndRun(math.dot);
    maths.dot = function (m1, m2) {
        return maths.mulMatrix(m1, m2);
    };
    //https://en.wikipedia.org/wiki/Norm_(mathematics)#Euclidean_norm
    //  var norm = logAndRun(math.norm);
    maths.vectorLen = function (v) {
        return Math.hypot.apply(Math, v);
    };
    //math.divide = logAndRun(math.divide);
    maths.divItems = function (items, divisor) {
        return items.map(function (x) { return x / divisor; });
    };
    //var dotPow = logAndRun(math.dotPow);
    maths.squareItems = function (items) {
        return items.map(function (x) { return x * x; });
    };
    maths.normalize = function (v) {
        return this.divItems(v, this.vectorLen(v));
    };
    return maths;
}());
__reflect(maths.prototype, "maths");
var bezier = (function () {
    function bezier() {
    }
    //Evaluates cubic bezier at t, return point
    bezier.q = function (ctrlPoly, t) {
        var tx = 1.0 - t;
    ...
    bezier.qprime = function (ctrlPoly, t) {
        var tx = 1.0 - t;
    ...
    bezier.qprimeprime = function (ctrlPoly, t) {
